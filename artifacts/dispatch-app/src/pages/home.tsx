...
import { MapPin, Navigation, User, Phone, CheckCircle2, Info, Loader2 } from "lucide-react";
...
const FARE_MATRIX: Record<string, Record<string, number>> = {
  "Port of Spain": { "Diego Martin": 40, "San Fernando": 120, "Chaguanas": 80, "Arima": 65, "Sangre Grande": 110 },
  "Diego Martin": { "Port of Spain": 40, "San Fernando": 140, "Chaguanas": 95, "Arima": 85, "Sangre Grande": 130 },
  "San Fernando": { "Port of Spain": 120, "Diego Martin": 140, "Chaguanas": 60, "Arima": 100, "Sangre Grande": 150 },
  "Chaguanas": { "Port of Spain": 80, "Diego Martin": 95, "San Fernando": 60, "Arima": 55, "Sangre Grande": 90 },
  "Arima": { "Port of Spain": 65, "Diego Martin": 85, "San Fernando": 100, "Chaguanas": 55, "Sangre Grande": 60 },
  "Sangre Grande": { "Port of Spain": 110, "Diego Martin": 130, "San Fernando": 150, "Chaguanas": 90, "Arima": 60 },
};

function calculateFare(pickup: string, dropoff: string): number {
  if (!pickup || !dropoff || pickup === dropoff) return 0;
  return FARE_MATRIX[pickup]?.[dropoff] ?? 50;
}
...
export default function Home() {
  ...
  const [bookedJob, setBookedJob] = useState<{ id: string; name: string; pickup: string; dropoff: string; deposit: number } | null>(null);
    ...
    createJob.mutate(
      ...
      },
      {
        onSuccess: (job) => {
          ...
          setBookedJob({ id: job.id, name, pickup, dropoff, deposit });
        },
      }
    );
  };
  ...
  if (bookedJob) {
    return (
      <div className="min-h-screen bg-amber-50 flex items-center justify-center p-4" style={{ fontFamily: "'Outfit', sans-serif" }}>
        <Card className="w-full max-w-md shadow-xl border-amber-200">
          <CardContent className="pt-10 pb-8 flex flex-col items-center text-center">
            ...
            <p className="text-teal-800/70 mb-2">
              Thank you, <strong>{bookedJob.name}</strong>. Your ride from{" "}
              <strong>{bookedJob.pickup}</strong> to <strong>{bookedJob.dropoff}</strong> is booked.
            </p>
            <p className="text-xs text-teal-600/60 font-mono mb-6">Ref: {bookedJob.id}</p>
            <p className="text-sm font-medium text-amber-700 bg-amber-100 px-4 py-2 rounded-full mb-8">
              Deposit Paid: TTD {bookedJob.deposit.toFixed(2)}
            </p>
            <p className="text-sm text-teal-700/70 mb-8">
              A driver will be assigned shortly. Please keep your phone nearby.
            </p>
            <Button
  ...
  return (
    <div className="min-h-screen bg-[#FFFBF4] text-teal-950" style={{ fontFamily: "'Outfit', sans-serif" }}>
      ...
      <header className="bg-teal-900 text-amber-50 py-4 px-6 md:px-12 flex items-center justify-between shadow-md">
        <div className="flex items-center gap-2">
          <div className="bg-amber-500 w-8 h-8 rounded flex items-center justify-center shadow-inner">
            <span className="font-bold text-teal-900 text-sm">M</span>
          </div>
        ...
        <nav className="hidden md:flex gap-6 text-sm font-medium text-teal-100">
          ...
          <a href="#routes" className="hover:text-amber-400 transition-colors">Routes</a>
          ...
          <div className="absolute bottom-0 lg:bottom-auto lg:top-1/3 left-0 w-full lg:w-11/12 p-6 lg:p-12 z-10">
            <div className="bg-[#FFFBF4]/80 backdrop-blur-md p-6 lg:p-8 rounded-2xl shadow-2xl border border-white/50 inline-block">
              <h2 className="text-3xl lg:text-5xl font-extrabold text-teal-900 leading-tight mb-4">
                ...
                <span className="text-amber-600">across the island.</span>
              </h2>
            ...
            <CardContent>
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="space-y-4 relative">
                  <div className="absolute left-[15px] top-[32px] bottom-[32px] w-[2px] bg-amber-200 z-0 hidden md:block" />
                  ...
                  <div className="space-y-2 relative z-10">
                    ...
                    <select
                      ...
                      className="w-full h-12 px-4 rounded-xl border-2 border-teal-100 bg-white focus:border-teal-500 focus:outline-none text-teal-900 transition-colors"
                      value={pickup}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
                  <div className="space-y-2">
                    <Label htmlFor="name" className="text-teal-900 font-medium">Passenger Name</Label>
                    <div className="relative">
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="phone" className="text-teal-900 font-medium">Phone Number</Label>
                    <div className="relative">
                ...
                <div
                  className={`transition-all duration-500 ease-in-out overflow-hidden rounded-2xl border ${
                    fare > 0
                      ? "bg-amber-50/80 border-amber-200 p-5 opacity-100 max-h-96"
                      ...
                      <div className="flex gap-3 items-start bg-white/60 p-3 rounded-xl border border-amber-100">
                        ...
                        <div>
                          <p className="text-sm font-semibold text-teal-900">
                            25% Deposit Required: TTD {deposit.toFixed(2)}
                          </p>
                          <p className="text-xs text-teal-700/80 mt-1 leading-relaxed">
                            To confirm your booking and hold your seat, a deposit is required now. The remaining balance of TTD {(fare - deposit).toFixed(2)} is paid directly to the driver.
                          </p>
                        </div>
                      </div>
                    </div>
                ...
                <Button
                  ...
                  disabled={!fare || !name || !phone || createJob.isPending}
                  className="w-full h-14 text-lg font-semibold bg-amber-500 hover:bg-amber-600 text-teal-950 rounded-xl shadow-lg shadow-amber-500/20 transition-all active:scale-[0.98] disabled:opacity-50"
                >
                  {createJob.isPending ? (
                    <span className="flex items-center gap-2">
                      <Loader2 className="w-5 h-5 animate-spin" /> Confirming your ride...
                    </span>
                  ) : fare > 0 ? (
                    `Pay TTD ${deposit.toFixed(2)} Deposit to Book`
                  ) : (
                    "Select Route to See Fare"
                  )}
                </Button>
      ...
      {/* Footer */}
      <footer id="contact" className="bg-teal-900 text-teal-300 text-sm py-8 px-6 md:px-12 text-center">
        <p className="text-amber-50 font-semibold mb-1">Maxi Hub TT</p>
        <p>Your trusted maxi taxi service across Trinidad &amp; Tobago</p>
      </footer>
    </div>
  );
}
