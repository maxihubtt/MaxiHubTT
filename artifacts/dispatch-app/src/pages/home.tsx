    ...
    <div className="min-h-screen bg-[#FFFBF4] text-teal-950" style={{ fontFamily: "'Outfit', sans-serif" }}>
      ...
      <section className="py-16 px-6 md:px-12 bg-white">
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-10">
            <div className="h-0.5 w-16 mx-auto mb-4 rounded-full" style={{ background: "linear-gradient(90deg, #ce1126, #000, #ce1126)" }} />
            <h2 className="text-3xl font-black text-teal-900 mb-2">What People Say</h2>
            <p className="text-teal-700/70 text-sm">Real riders, real experiences across Trinidad & Tobago.</p>
          </div>
            ...
            {[
              {
                ...
                quote: "Booked for our family beach lime to Maracas. Driver was on time, vehicle was spotless. No stress at all — will definitely use again!",
                trip: "Beach Lime · Maracas Bay",
                initials: "KA",
                color: "bg-teal-600",
              },
              {
                ...
                quote: "Used Maxi Hub TT for an airport run. Fixed price, no haggling, no surprises. The driver even helped with the luggage. Highly recommend.",
                trip: "Airport Run · Piarco",
                initials: "MT",
                color: "bg-amber-500",
              },
              {
                name: "Sherise M.",
                location: "Chaguanas",
                quote: "Organised a group event maxi for 14 of us. Seamless from booking to drop-off. Love the transparency with the pricing — you know exactly what you paying.",
                trip: "Group Event · Central",
                initials: "SM",
                color: "bg-sky-600",
              },
            ].map(({ name, location, quote, trip, initials, color }) => (
              <div key={name} className="bg-[#FFFBF4] border border-amber-100 rounded-2xl p-6 flex flex-col gap-4">
                ...
                <p className="text-sm text-teal-800 leading-relaxed flex-1">"{quote}"</p>
                <div className="flex items-center gap-3 pt-2 border-t border-amber-100">
                  <div className={`w-9 h-9 rounded-full flex items-center justify-center text-white text-xs font-black shrink-0 ${color}`}>
                    {initials}
                  </div>
                  <div>
                    <p className="text-sm font-bold text-teal-900 leading-none">{name}</p>
                    <p className="text-xs text-teal-500 mt-0.5">{location} · {trip}</p>
                  </div>
                </div>
              </div>
            ))}
